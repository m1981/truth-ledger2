...
| `scripts/tl2 mirror` | client | CMP-005 | 2026-09-01 | 2026-09-01, seeded fixture: a manifest id cited nowhere → GAP, a cited id absent from the manifest → ORPHAN, both red (exit 1); re-witnessed at v0.1.1 on a two-family corpus: sibling ignored, own GAP and same-family ORPHAN still red; re-witnessed at v0.1.2 (CMP-020) on a layout-faithful fixture: a deleted test citation now GAPs despite the mandatory inline spec id, --cite-all reproduces the old blindness | 2026-09-01: matched three-id fixture → clean; v0.1.1: two-family fixture with sibling present → clean | 0 | 28 (v0.1.0 sibling-family false orphans, fixed in v0.1.1) + 1 structural false-negative class (CMP-020: the GAP direction dead under inline ids, measured by the effectiveness review, fixed in v0.1.2 — citations count only from test-shaped files; the five real consumer manifests stay clean under the fixed scan, so the discipline there was real, not masked) | 2026-12-31 |
...
| `scripts/separation-canary` | engine-dev | CMP-010 | 2026-09-01 | 2026-09-01, two seeded leaks, each caught: an in-sandbox process opening an engine file → RED AUDIT (detector 1); a file created inside the engine repo → RED on two detectors at once (mtime sweep + status diff) | 2026-09-01: full client cycle (all seven verbs) in a sandbox → green, engine untouched, no out-of-sandbox opens. Declared limit: subprocess (git) reads untraced — compensated by the write detectors | 1 (2026-09-01: BLOCKED the v0.1.2 release — the citation-scope change had broken the canary's own client-cycle fixture, a non-test-shaped filename; caught at the release gate, fixed, release re-run) | 0 | 2026-12-31 |
